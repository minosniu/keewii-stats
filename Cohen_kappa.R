...
filenames_brac<-list.files(dirs_brac,log_filename, recursive=TRUE, full.names=TRUE)
dirs_fpb<-grep("[B]+$",list.dirs(cur_path,recursive=FALSE),value=TRUE)
filenames_fpb<-list.files(dirs_fpb,log_filename, recursive=TRUE, full.names=TRUE) 
...
for (a in 1:numfile){
  ...
  temp<-read.table(filenames_fpb[2*a-1],header=FALSE,nrows=num_trials,encoding='UTF-8')[,2:3]
  temp_fpb_cart<-rbind(temp_fpb_cart,temp)
  
  temp<-read.table(filenames_fpb[2*a],header=FALSE,nrows=num_trials,encoding='UTF-8') [,2:3] 
...
for (a in 1:len_vowel){
  for (b in 1:len_vowel){
    ...
    if (temp_fpb_cart[k,1]==Vowel[b] & temp_fpb_cart[k,2]==Vowel[a]){
      Cohen_FC[a,b]<-Cohen_FC[a,b]+1
    }
    if (temp_fpb_polar[k,1]==Vowel[b] & temp_fpb_polar[k,2]==Vowel[a]){
      Cohen_FP[a,b]<-Cohen_FP[a,b]+1
    }   
  }
}
Kappa.test(Cohen_BC, y=NULL, conf.level=0.95)
Kappa.test(Cohen_BP, y=NULL, conf.level=0.95)
Kappa.test(Cohen_FC, y=NULL, conf.level=0.95)
Kappa.test(Cohen_FP, y=NULL, conf.level=0.95)
